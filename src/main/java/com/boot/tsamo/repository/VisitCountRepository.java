...
public interface VisitCountRepository extends JpaRepository<VisitCount, Long> {
    ...
    @Query("SELECT DISTINCT v.visitDate FROM VisitCount v WHERE v.visitDate >= :startDate ORDER BY v.visitDate DESC")
    ...
    // 날짜별 방문자 수를 집계하는 메서드
    @Query("SELECT v.visitDate, COUNT(v) FROM VisitCount v WHERE v.visitDate >= :startDate GROUP BY v.visitDate ORDER BY v.visitDate DESC")
